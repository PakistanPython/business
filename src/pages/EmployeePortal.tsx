...
import { Clock, MapPin, Calendar, Users, AlertCircle, CheckCircle, RefreshCw, Building2, Trophy, Target, BarChart3, Activity } from 'lucide-react';
...
const EmployeePortal: React.FC = () => {
  ...
  const { first_name, last_name, position, department, employee_code, hire_date, status } = employeeData;
  const initials = `${first_name?.[0] ?? ''}${last_name?.[0] ?? ''}`.toUpperCase();
  const isClockedIn = Boolean(userTodayAttendance?.clock_in_time && !userTodayAttendance?.clock_out_time);
              ...
              <CardContent className="p-4 space-y-3">
                {userTodayAttendance ? (
                    ...
                    <div className="rounded-lg border p-3 bg-slate-50">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                        <div className="rounded bg-white p-3 border border-green-100">
                          <p className="text-green-700">Clock_In</p>
                          <p className="text-lg font-bold text-green-800 mt-1">{formatTime(userTodayAttendance.clock_in_time)}</p>
                        </div>
                        <div className="rounded bg-white p-3 border border-red-100">
                          <p className="text-red-700">Clock_Out</p>
                          <p className="text-lg font-bold text-red-800 mt-1">{formatTime(userTodayAttendance.clock_out_time)}</p>
                        </div>
                        <div className="rounded bg-white p-3 border border-blue-100">
                          <p className="text-blue-700">Hours</p>
                          <p className="text-lg font-bold text-blue-800 mt-1">{formatDuration(userTodayAttendance.total_hours)}</p>
                        </div>
                      </div>
                    </div>

                    {(userTodayAttendance.late_minutes > 0 || userTodayAttendance.location_latitude || userTodayAttendance.location_longitude) && (
                      <div className="rounded-lg border p-3 bg-slate-50">
                  ...
                  <div className="rounded-lg border p-4 bg-slate-50 text-sm text-slate-500">
                    No attendance recorded for today. Clock in to get started!
                  </div>
                )}

                {/* Single Switch Clock Button */}
                <div className="mt-3">
                  <Button
                    onClick={isClockedIn ? handleClockOut : handleClockIn}
                    disabled={clockingIn || clockingOut}
                    className={`w-full h-16 text-lg font-semibold transition-all duration-300 shadow-lg hover:shadow-xl ${
                      isClockedIn
                        ? 'bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 hover:scale-[1.02]'
