...
const testInvoices = [
  {
    business_id: 1, // You'll need to adjust this to match your user ID
  ...
  },
  {
    ...
    invoice_date: new Date(Date.now() - 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 45 days ago
...
const testPayments = [
  {
    ...
    record_id: 2, // Small Business Inc invoice
  ...
  },
  {
    business_id: 1,
    record_type: 'receivable',
    record_id: 3, // Enterprise Solutions invoice
    payment_date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 5 days ago
    amount: 3000.00,
  ...
  },
  {
    business_id: 1,
    record_type: 'receivable',
    record_id: 3, // Enterprise Solutions invoice (second payment)
    payment_date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 2 days ago
    amount: 2500.00,
    payment_method: 'Bank Transfer',
...
export const seedTestData = async () => {
  try {
    ...
    // Update paid amounts and status for invoices with payments
    await dbRun(`
...
// To run this seeder, add it to your backend startup or create an endpoint