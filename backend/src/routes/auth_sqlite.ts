...
import { dbGet, dbRun } from '../config/database_sqlite';
...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (userType === 'employee') {
      // For employees, get data from employees table
      user = await dbGet(
        'SELECT id, employee_code, first_name, last_name, email, phone, address, department, position, employment_type, salary_type, hire_date, status, business_id, created_at FROM employees WHERE id = ?',
        [userId]
      );
      
      if (user) {
        // Format employee data to match expected user structure
        user = {
          ...user,
          user_type: 'employee',
          username: user.email,
      ...
      // For business owners/admins, get data from users table
      user = await dbGet(
        'SELECT id, username, email, full_name, business_name, user_type, created_at FROM users WHERE id = ?',
        [userId]
      );
    }
...
router.put('/profile', [
  ...
  try {
    ...
    // Get updated user
    const user = await dbGet(
      'SELECT id, username, email, full_name, business_name, created_at FROM users WHERE id = ?',
      [userId]
    );
