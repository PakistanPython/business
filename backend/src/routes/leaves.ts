...
(async () => {
  try {
    ...
    await dbRun(`ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS max_days_per_month INTEGER`);
    await dbRun(`ALTER TABLE leave_types ADD COLUMN IF NOT EXISTS leave_limit_period TEXT DEFAULT 'year'`);
...
router.post('/types', async (req: Request, res: Response) => {
  try {
    ...
    const result = await dbRun(
      ...
      [name, description, yearlyLimit, monthlyLimit, period, is_paid, businessId]
    );
    ...
    const result = await dbRun(
      `UPDATE leave_types
       SET name = $1,
           ...
           business_id = $7
       WHERE id = $8`,
      [name, description, yearlyLimit, monthlyLimit, period, is_paid, businessId, id]
    );
...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    const employee = await dbGet(
      'SELECT id, first_name, last_name, hire_date FROM employees WHERE id = $1 AND business_id = $2',
    ...
    const leaveTypesRaw = await dbAll(
      `SELECT *
       FROM leave_types
       WHERE business_id = $1 OR business_id IS NULL
       ORDER BY business_id DESC NULLS LAST, id DESC`,
      [businessId]
    );

    // Prefer business specific leave type over default one with same name
    const leaveTypesByName = new Map<string, any>();
    for (const lt of leaveTypesRaw) {
      if (!leaveTypesByName.has(lt.name)) leaveTypesByName.set(lt.name, lt);
    }
    ...
    const overlapDays = (startA: Date, endA: Date, startB: Date, endB: Date): number => {
    ...
    const getCompletedMonthsSince = (start: Date, end: Date): number => {
      let months = (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth());
      if (end.getDate() < start.getDate()) months -= 1;
      return Math.max(0, months);
    };

    const getCompletedYearsSince = (start: Date, end: Date): number => {
    ...
    // - Monthly leaves start only AFTER completing 3 months from hire date.
    // - Annual leaves are added only AFTER each completed year from hire date.
    const monthPeriodsAccrued = Math.max(0, completedMonthsSinceHire - 2);
    const yearPeriodsAccrued = completedYearsSinceHire;

    const summary = leaveTypes.map((leaveType: any) => {
      ...
      const matchedLeaves = leaves.filter((l: any) => l.leave_type === leaveType.name);

      const usedTotal = matchedLeaves.reduce((sum: number, leave: any) => {
        const leaveStart = new Date(leave.start_date);
        const leaveEnd = new Date(leave.end_date);
        if (Number.isNaN(leaveStart.getTime()) || Number.isNaN(leaveEnd.getTime())) return sum;
        return sum + overlapDays(leaveStart, leaveEnd, safeHireDate, now);
      }, 0);

      const usedCurrentPeriod = matchedLeaves.reduce((sum: number, leave: any) => {
        const leaveStart = new Date(leave.start_date);
        const leaveEnd = new Date(leave.end_date);
        if (Number.isNaN(leaveStart.getTime()) || Number.isNaN(leaveEnd.getTime())) return sum;
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Validate leave allowance based on leave type period (month/year)
    const leaveTypeConfig = await dbGet(
      `SELECT id, name, leave_limit_period, max_days_per_year, max_days_per_month
       FROM leave_types
       WHERE name = $1 AND (business_id = $2 OR business_id IS NULL)
       ORDER BY business_id DESC NULLS LAST
       LIMIT 1`,
      [leave_type, businessId]
    );
    ...
    if (leaveTypeConfig) {
      ...
      if (limit > 0) {
        ...
        const overlappingLeaves = await dbAll(
          `SELECT start_date, end_date
           ...
           WHERE employee_id = $1
             ...
             AND start_date <= $3
             AND end_date >= $4`,
          [targetEmployeeId, leave_type, fmt(periodEnd), fmt(periodStart)]
        );

        const overlapDaysInPeriod = (startA: Date, endA: Date, startB: Date, endB: Date) => {
          const start = new Date(Math.max(startA.getTime(), startB.getTime()));
          const end = new Date(Math.min(endA.getTime(), endB.getTime()));
          if (end < start) return 0;
          return Math.floor((end.getTime() - start.getTime()) / msPerDay) + 1;
        ...
        const alreadyUsed = overlappingLeaves.reduce((sum: number, row: any) => {
          const rowStart = new Date(row.start_date);
          const rowEnd = new Date(row.end_date);
          if (Number.isNaN(rowStart.getTime()) || Number.isNaN(rowEnd.getTime())) return sum;
...
router.put('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    const leave = await dbGet(
      `SELECT l.*, e.business_id, e.first_name, e.last_name, e.employee_code
       FROM leaves l
       JOIN employees e ON l.employee_id = e.id
       WHERE l.id = $1 AND e.business_id = $2`,
      [id, businessId]
    );
    ...
    if (status === 'approved') {
      const leaveTypeConfig = await dbGet(
        `SELECT leave_limit_period, max_days_per_year, max_days_per_month
         FROM leave_types
         WHERE name = $1 AND (business_id = $2 OR business_id IS NULL)
         ORDER BY business_id DESC NULLS LAST
         LIMIT 1`,
        [leave.leave_type, businessId]
      );
      ...
      if (leaveTypeConfig) {
        ...
        if (limit > 0 && !Number.isNaN(requestStart.getTime()) && !Number.isNaN(requestEnd.getTime())) {
          ...
          const approvedInPeriod = await dbAll(
            `SELECT start_date, end_date
             FROM leaves
             WHERE employee_id = $1
               AND leave_type = $2
               ...
               AND start_date <= $4
               AND end_date >= $5`,
            [leave.employee_id, leave.leave_type, id, formatDateOnly(periodEnd), formatDateOnly(periodStart)]
          );

          const alreadyUsed = approvedInPeriod.reduce((sum: number, row: any) => {
            const rowStart = new Date(row.start_date);
            const rowEnd = new Date(row.end_date);
            if (Number.isNaN(rowStart.getTime()) || Number.isNaN(rowEnd.getTime())) return sum;
            ...
            await dbRun(
              `UPDATE attendance
               SET status = 'absent',
                   ...
                   updated_at = NOW()
               WHERE id = $2`,
              [leaveNote, existingAttendance.id]
            );
          } else {
            await dbRun(
              `INSERT INTO attendance
                (employee_id, date, status, total_hours, overtime_hours, notes)
               VALUES ($1, $2, 'absent', 0, 0, $3)`,
              [leave.employee_id, dateKey, leaveNote]
            );
          }
        }
      }
