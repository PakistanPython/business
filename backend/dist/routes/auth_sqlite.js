...
router.post('/register', [
    ...
    try {
        ...
        const existingUser = await (0, database_sqlite_1.dbGet)('SELECT id FROM users WHERE username = ? OR email = ?', [username, email]);
        if (existingUser) {
            return res.status(400).json({
                success: false,
        ...
        const result = await (0, database_sqlite_1.dbRun)('INSERT INTO users (username, email, password_hash, full_name, business_name) VALUES (?, ?, ?, ?, ?)', [username, email, passwordHash, full_name, business_name || null]);
        ...
        for (const category of defaultCategories) {
            await (0, database_sqlite_1.dbRun)('INSERT INTO categories (user_id, name, type, color, icon) VALUES (?, ?, ?, ?, ?)', [userId, category.name, category.type, category.color, category.icon]);
        }
        await (0, database_sqlite_1.dbRun)('INSERT INTO accounts (user_id, account_type, account_name, balance) VALUES (?, ?, ?, ?)', [userId, 'cash', 'Cash', 0]);
        const user = await (0, database_sqlite_1.dbGet)('SELECT id, username, email, full_name, business_name, user_type, business_id, created_at FROM users WHERE id = ?', [userId]);
...
router.post('/login', [
    ...
    try {
        ...
        const user = await (0, database_sqlite_1.dbGet)('SELECT * FROM users WHERE username = ? OR email = ?', [login, login]);
        if (!user) {
...
router.get('/profile', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (userType === 'employee') {
            user = await (0, database_sqlite_1.dbGet)('SELECT id, employee_code, first_name, last_name, email, phone, address, department, position, employment_type, salary_type, hire_date, status, business_id, created_at FROM employees WHERE id = ?', [userId]);
            if (user) {
                user = {
                    ...user,
                    user_type: 'employee',
            ...
            user = await (0, database_sqlite_1.dbGet)('SELECT id, username, email, full_name, business_name, user_type, created_at FROM users WHERE id = ?', [userId]);
        }
        if (!user) {
...
router.put('/profile', [
    ...
    try {
        ...
        if (email) {
            const existingUser = await (0, database_sqlite_1.dbGet)('SELECT id FROM users WHERE email = ? AND id != ?', [email, userId]);
            if (existingUser) {
                return res.status(400).json({
                    success: false,
        ...
        values.push(userId);
        await (0, database_sqlite_1.dbRun)(`UPDATE users SET ${updates.join(', ')} WHERE id = ?`, values);
        const user = await (0, database_sqlite_1.dbGet)('SELECT id, username, email, full_name, business_name, created_at FROM users WHERE id = ?', [userId]);
        res.json({
            success: true,
            data: {
                user
...
router.post('/employee/login', [
    ...
    try {
        ...
        const employee = await (0, database_sqlite_1.dbGet)('SELECT * FROM employees WHERE email = ? AND status = "active"', [email]);
        if (!employee) {
            return res.status(401).json({
                success: false,
