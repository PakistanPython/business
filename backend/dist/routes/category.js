...
router.get('/:id', async (req, res) => {
    try {
        ...
        const category = await (0, database_1.dbGet)('SELECT id, name, type, color, icon, created_at FROM categories WHERE id = ? AND user_id = ?', [categoryId, userId]);
        if (!category) {
...
router.post('/', [
    ...
    try {
        ...
        const existingCategory = await (0, database_1.dbGet)('SELECT id FROM categories WHERE user_id = ? AND name = ? AND type = ?', [userId, name, type]);
        if (existingCategory) {
...
router.put('/:id', [
    ...
    try {
        ...
        const existingCategory = await (0, database_1.dbGet)('SELECT id, name, type FROM categories WHERE id = ? AND user_id = ?', [categoryId, userId]);
        if (!existingCategory) {
            return res.status(404).json({
                success: false,
                message: 'Category not found'
        ...
        if (name && name !== existingCategory.name) {
            const duplicateCategory = await (0, database_1.dbGet)('SELECT id FROM categories WHERE user_id = ? AND name = ? AND type = ? AND id != ?', [userId, name, existingCategory.type, categoryId]);
        ...
        const updatedCategory = await (0, database_1.dbGet)('SELECT * FROM categories WHERE id = ?', [categoryId]);
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const category = await (0, database_1.dbGet)('SELECT id, name, type FROM categories WHERE id = ? AND user_id = ?', [categoryId, userId]);
        if (!category) {
            return res.status(404).json({
                success: false,
                message: 'Category not found'
        ...
        const incomeUsage = await (0, database_1.dbGet)('SELECT COUNT(*) as count FROM income WHERE user_id = ? AND category = ?', [userId, category.name]);
        const expenseUsage = await (0, database_1.dbGet)('SELECT COUNT(*) as count FROM expenses WHERE user_id = ? AND category = ?', [userId, category.name]);
        const purchaseUsage = await (0, database_1.dbGet)('SELECT COUNT(*) as count FROM purchases WHERE user_id = ? AND category = ?', [userId, category.name]);
        ...
        await (0, database_1.dbRun)('DELETE FROM categories WHERE id = ? AND user_id = ?', [categoryId, userId]);
        res.json({
            success: true,
