...
router.get('/summary', async (req, res) => {
    try {
        ...
        const incomeTotal = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM income WHERE user_id = ?', [userId]);
        const expenseTotal = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE user_id = ?', [userId]);
        const purchaseTotal = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM purchases WHERE user_id = ?', [userId]);
        const salesTotal = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(selling_price), 0) as revenue, COALESCE(SUM(profit), 0) as profit FROM sales WHERE user_id = ? AND status = "completed"', [userId]);
        const purchaseCount = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM purchases WHERE user_id = ?', [userId]);
        const salesCount = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM sales WHERE user_id = ? AND status = "completed"', [userId]);
        const accountsBalance = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(balance), 0) as total FROM accounts WHERE user_id = ?', [userId]);
        const activeLoans = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(current_balance), 0) as total FROM loans WHERE user_id = ? AND status = "active"', [userId]);
        const charityRequired = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount_required), 0) as total FROM charity WHERE user_id = ?', [userId]);
        const charityPaid = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount_paid), 0) as total FROM charity WHERE user_id = ?', [userId]);
        const charityRemaining = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount_remaining), 0) as total FROM charity WHERE user_id = ?', [userId]);
        const arTotal = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM accounts_receivable WHERE business_id = ?', [userId]);
        const arOutstanding = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(balance_amount), 0) as total FROM accounts_receivable WHERE business_id = ? AND status != "paid"', [userId]);
        const arOverdue = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(balance_amount), 0) as total FROM accounts_receivable WHERE business_id = ? AND due_date < date("now") AND status != "paid"', [userId]);
        const arInvoicesCount = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM accounts_receivable WHERE business_id = ?', [userId]);
        const arOverdueCount = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM accounts_receivable WHERE business_id = ? AND due_date < date("now") AND status != "paid"', [userId]);
        ...
        summary.total_accounts_receivable = parseFloat(arTotal?.total || 0);
        summary.total_ar_outstanding = parseFloat(arOutstanding?.total || 0);
        summary.total_ar_overdue = parseFloat(arOverdue?.total || 0);
        summary.ar_invoices_count = parseInt(arInvoicesCount?.count || 0);
        summary.ar_overdue_count = parseInt(arOverdueCount?.count || 0);
        summary.net_worth = summary.total_income - summary.total_expenses + summary.total_ar_outstanding;
        summary.available_cash = summary.total_accounts_balance - summary.total_active_loans;
        const monthlyData = await (0, database_sqlite_1.dbAll)(`
      WITH months AS (
    ...
    `, [userId, userId, userId]);
        const recentTransactions = await (0, database_sqlite_1.dbAll)(`
      SELECT 
      ...
      LIMIT 10
    `, [userId]);
        const topExpenseCategories = await (0, database_sqlite_1.dbAll)(`
      SELECT 
        category,
      ...
      LIMIT 5
    `, [userId, userId]);
        const trendData = await (0, database_sqlite_1.dbAll)(`
    ...
    `, [userId, userId]);
        const charityOverview = await (0, database_sqlite_1.dbAll)(`
      SELECT 
        status,
        COUNT(*) as count,
...
router.get('/analytics', [
    ...
    try {
        ...
        const incomeAnalytics = await (0, database_sqlite_1.dbAll)(`
      SELECT 
        ${groupBy} as period,
        SUM(amount) as total_amount,
        COUNT(*) as transaction_count,
    ...
    `, [userId]);
        const expenseAnalytics = await (0, database_sqlite_1.dbAll)(`
      SELECT 
        ${groupBy} as period,
        SUM(amount) as total_amount,
        COUNT(*) as transaction_count,
      ...
      ORDER BY period, total_amount DESC
    `, [userId]);
        const profitAnalysis = await (0, database_sqlite_1.dbAll)(`
      SELECT 
        period,
...
router.get('/metrics', async (req, res) => {
    try {
        ...
        const revenue30d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM income WHERE user_id = ? AND date >= date("now", "-30 days")', [userId]);
        const revenue90d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM income WHERE user_id = ? AND date >= date("now", "-90 days")', [userId]);
        const expenses30d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE user_id = ? AND date >= date("now", "-30 days")', [userId]);
        const expenses90d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE user_id = ? AND date >= date("now", "-90 days")', [userId]);
        const avgIncome30d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(AVG(amount), 0) as avg FROM income WHERE user_id = ? AND date >= date("now", "-30 days")', [userId]);
        const avgExpense30d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(AVG(amount), 0) as avg FROM expenses WHERE user_id = ? AND date >= date("now", "-30 days")', [userId]);
        const charityPaid30d = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount_paid), 0) as total FROM charity WHERE user_id = ? AND payment_date >= date("now", "-30 days")', [userId]);
        const charityPending = await (0, database_sqlite_1.dbGet)('SELECT COALESCE(SUM(amount_remaining), 0) as total FROM charity WHERE user_id = ? AND status != "paid"', [userId]);
        const totalAccounts = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM accounts WHERE user_id = ?', [userId]);
        const activeLoans = await (0, database_sqlite_1.dbGet)('SELECT COUNT(*) as count FROM loans WHERE user_id = ? AND status = "active"', [userId]);
        ...
        const currentMonth = await (0, database_sqlite_1.dbGet)(`
      SELECT 
    ...
    `, [userId, userId]);
        const previousMonth = await (0, database_sqlite_1.dbGet)(`
      SELECT 
        COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
        COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expenses
      FROM (
